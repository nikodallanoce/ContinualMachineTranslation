...
def calcolate_bleu(translation_ds):
    ...
    test_loader = DataLoader(translation_ds, num_workers=4, batch_size=64, drop_last=True, pin_memory=True)
    trans_fr = []
    original_txt = []
    for i, batch in enumerate(tqdm(test_loader)):
        translation = model.generate(batch['input_ids'].to(dev), max_length=128, decoder_start_token_id=250008,
                                     num_beams=5)
...
if __name__ == '__main__':
    ...
    dev = "cuda:0"
    model: MBartForConditionalGeneration = MBartForConditionalGeneration(mbart_config).to(dev)

    model.load_state_dict(
        torch.load(
            "/home/n.dallanoce/PyCharm/pretraining/weights/mbart_ft_fr-en_cc/checkpoint-380000/pytorch_model.bin",
            map_location=dev))
